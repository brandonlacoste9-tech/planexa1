...
function ServiceStep({ appointmentTypes, onSelect }: { appointmentTypes: AppointmentType[]; onSelect: (t: AppointmentType) => void }) {
  ...
  const paid = active.filter(t => t.price_cents > 0);
  const free = active.filter(t => t.price_cents === 0);
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between gap-2 mb-1">
            ...
            <span className="px-2 py-1 rounded-full text-xs font-semibold" style={{ backgroundColor: type.color_hex, color: 'white', fontFamily: 'DM Sans, sans-serif' }}>
              {type.price_cents > 0 ? formatPrice(type.price_cents) : 'FREE'}
            </span>
          </div>
  ...
  return (
    <div className="space-y-5">
      <h2 className="text-lg font-medium" style={{ fontFamily: 'DM Sans, sans-serif', color: '#1E293B' }}>Select a service</h2>
      {paid.length > 0 && (
        <div>
          <div className="text-xs font-semibold mb-3" style={{ color: '#2D6A4F', fontFamily: 'DM Sans, sans-serif' }}>💳 PAID SERVICES</div>
          <div className="space-y-3">{paid.map(t => <ServiceCard key={t.id} type={t} />)}</div>
        </div>
      )}
      {free.length > 0 && (
        <div>
          <div className="text-xs font-semibold mb-3" style={{ color: '#64748B', fontFamily: 'DM Sans, sans-serif' }}>🎁 FREE SERVICES</div>
          <div className="space-y-3">{free.map(t => <ServiceCard key={t.id} type={t} />)}</div>
        </div>
      )}
    </div>
  );
...
function DateStep({ selectedDate, enabledDays, onSelect }: { selectedDate: Date | null; enabledDays: number[]; onSelect: (d: Date) => void }) {
  ...
  return (
    <div className="space-y-3">
      <h2 className="text-lg font-medium" style={{ fontFamily: 'DM Sans, sans-serif', color: '#1E293B' }}>Pick a date</h2>
      <div className="flex items-center justify-between mb-4">
        <button onClick={() => setCurrentMonth(new Date(year, month - 1))} className="p-1 rounded" style={{ color: '#2D6A4F' }}><ChevronLeft size={18} /></button>
        <span className="text-sm font-semibold" style={{ color: '#1E293B', fontFamily: 'DM Sans, sans-serif' }}>
          {currentMonth.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
        </span>
        <button onClick={() => setCurrentMonth(new Date(year, month + 1))} className="p-1 rounded" style={{ color: '#2D6A4F' }}><ChevronRight size={18} /></button>
      </div>
        ...
        {Array.from({ length: daysInMonth }, (_, i) => {
          ...
          return (
            <button
              ...
              style={{
                backgroundColor: isSelected ? '#2D6A4F' : disabled ? '#F3F4F6' : 'white',
...
// ─── Time Step ────────────────────────────────────────────────────────────────
function TimeStep({ slots, isLoading, selectedTime, onSelect }: { slots: string[]; isLoading: boolean; selectedTime: string | null; onSelect: (t: string) => void }) {
  if (isLoading) return (
    <div className="space-y-3">
      <h2 className="text-lg font-medium" style={{ fontFamily: 'DM Sans, sans-serif', color: '#1E293B' }}>Pick a time</h2>
      <div className="flex items-center justify-center py-8 gap-2" style={{ color: '#64748B' }}>
        <Loader2 size={16} className="animate-spin" />
        <span className="text-sm" style={{ fontFamily: 'DM Sans, sans-serif' }}>Loading available times…</span>
      </div>
    </div>
  );
  ...
  if (slots.length === 0) return (
    <div className="space-y-3">
      ...
      <div className="text-center py-8">
        <p className="text-sm" style={{ color: '#64748B', fontFamily: 'DM Sans, sans-serif' }}>No available times on this date.<br />Please pick a different day.</p>
      </div>
    </div>
  );
        ...
        {slots.map(time => {
          ...
          return (
            <button
              ...
              className="py-2 rounded text-xs font-medium transition-all"
              style={{
                backgroundColor: isSelected ? '#2D6A4F' : 'white',
...
function DetailsStep({ form, onChange }: { form: { name: string; email: string; phone: string; notes: string }; onChange: (field: string, value: string) => void }) {
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-medium mb-4" style={{ fontFamily: 'DM Sans, sans-serif', color: '#1E293B' }}>Your details</h2>
      <div className="space-y-3">
        {([['text', 'name', 'Full Name *', 'Alice Thompson'], ['email', 'email', 'Email Address *', 'alice@example.com'], ['tel', 'phone', 'Phone Number', '+1 416-555-0000']] as const).map(([type, field, label, placeholder]) => (
          <div key={field}>
        ...
        <div>
          <label className="block text-xs font-medium mb-1.5" style={{ color: '#475569', fontFamily: 'DM Sans, sans-serif' }}>Notes (optional)</label>
          <textarea value={form.notes} onChange={e => onChange('notes', e.target.value)} placeholder="Anything you'd like us to know…" rows={3} className="w-full planexa-input resize-none" />
        </div>
      </div>
    </div>
  );
...
function ConfirmationStep({ service, date, time, business, onBookAnother, onPaymentClick, isProcessing }: {
  ...
  return (
    <div className="text-center">
      <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-5" style={{ backgroundColor: '#D8F3DC' }}>
        <Check size={28} style={{ color: '#2D6A4F' }} strokeWidth={2.5} />
      </div>
      <h2 className="text-2xl mb-2" style={{ fontFamily: 'Fraunces, serif', fontWeight: 300, color: '#1E293B', fontStyle: 'italic' }}>You're all set!</h2>
      <p className="text-sm mb-6" style={{ color: '#64748B', fontFamily: 'DM Sans, sans-serif' }}>A confirmation has been sent to your email.</p>
      <div className="rounded-xl p-5 text-left mb-5" style={{ backgroundColor: '#F0EBE0', border: '1px solid #E8E0D0' }}>
      ...
      <div className="space-y-2">
        {service.price_cents > 0 && (
          <button onClick={onPaymentClick} disabled={isProcessing} className="w-full py-2.5 rounded-xl text-sm font-medium transition-colors flex items-center justify-center gap-2"
            style={{ backgroundColor: '#2D6A4F', color: 'white', fontFamily: 'DM Sans, sans-serif', opacity: isProcessing ? 0.7 : 1 }}
          ...
          >
            {isProcessing ? <><Loader2 size={14} className="animate-spin" />Processing…</> : <><DollarSign size={14} />Complete Payment</>}
          </button>
        )}
        <a href={`https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(service.name + ' with ' + business.name)}&dates=${toDateString(date).replace(/-/g, '')}T${time.replace(':', '')}00/${toDateString(date).replace(/-/g, '')}T${time.replace(':', '')}00`}
          ...
          style={{ border: '1px solid #E8E0D0', color: '#475569', fontFamily: 'DM Sans, sans-serif', backgroundColor: 'white', textDecoration: 'none' }}
        >
...
export default function BookingPage() {
  ...
  if (catalogQuery.error || !catalogQuery.data?.found) return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ backgroundColor: "#FAF7F2" }}>
      <p className="text-sm text-center max-w-sm" style={{ color: "#64748B", fontFamily: "DM Sans, sans-serif" }}>
        {catalogQuery.error ? 'Could not load this booking page.' : 'No booking page found for this link.'}
      </p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen py-10 px-4" style={{ backgroundColor: '#FAF7F2' }}>
      <div className="max-w-[520px] mx-auto">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium mb-4" style={{ backgroundColor: '#D8F3DC', color: '#2D6A4F', fontFamily: 'DM Sans, sans-serif' }}>
            <span className="pulse-dot" /> Booking available
          </div>
        ...
        <div className="flex gap-1 mb-8 justify-center">
          {stepOrder.map((s, idx) => (
            <div key={s} className="h-1 rounded-full transition-all" style={{ width: idx <= currentStepIdx ? '24px' : '8px', backgroundColor: idx <= currentStepIdx ? '#2D6A4F' : '#E8E0D0' }} />
          ))}
        </div>

        <div className="rounded-2xl p-6 mb-6" style={{ backgroundColor: 'white', border: '1px solid #E8E0D0' }}>
          {step === 'service' && (
            <ServiceStep appointmentTypes={appointmentTypes} onSelect={t => { setSelectedService(t); setStep('date'); }} />
          )}
          ...
          {step === 'time' && selectedService && (
            <>
              ...
              <TimeStep
                ...
                onSelect={t => { setSelectedTime(t); setStep('details'); }}
              />
            </>
          )}
          ...
          <div className="flex gap-3">
            {currentStepIdx > 0 && (
              <button onClick={() => setStep(stepOrder[currentStepIdx - 1])} className="flex-1 py-2.5 rounded-xl text-sm font-medium"
                style={{ border: '1px solid #E8E0D0', color: '#475569', fontFamily: 'DM Sans, sans-serif', backgroundColor: 'white' }}>
                Back
              </button>
            )}
            {step !== 'service' && step !== 'time' && (
              <button
                ...
                style={{
                  backgroundColor: '#2D6A4F', color: 'white', fontFamily: 'DM Sans, sans-serif',
                  opacity: (step === 'date' && !selectedDate) || createBookingMutation.isPending ? 0.5 : 1,
                }}
              >
                {createBookingMutation.isPending ? <><Loader2 size={14} className="animate-spin" />Booking…</> : step === 'details' ? 'Confirm Booking' : 'Continue'}
              </button>
            )}
          </div>
        )}

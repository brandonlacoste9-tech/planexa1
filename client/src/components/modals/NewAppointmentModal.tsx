// Planexa — NewAppointmentModal
// Design: White modal with cream-border, 16px radius, DM Sans form fields

import { useState } from 'react';
import { X, Calendar, Clock, User, Mail, FileText, Bell } from 'lucide-react';
import { toast } from 'sonner';
import { appointmentTypes, teamMembers, formatDuration, formatPrice } from '../../lib/data';
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
            ...
            >
              ...
              {appointmentTypes.filter(t => t.is_active).map(t => (
                <option key={t.id} value={t.id}>
                  {t.name} — {formatDuration(t.duration_minutes)} · {formatPrice(t.price_cents)}
                </option>
              ))}
            </select>
            {selectedType && (
          ...
          <div className="flex items-center justify-between py-1">
            ...
            <button
              type="button"
              onClick={() => setForm(f => ({ ...f, send_reminder: !f.send_reminder }))}
              className="relative w-10 h-5 rounded-full transition-colors"
              style={{
                backgroundColor: form.send_reminder ? '#2D6A4F' : '#CBD5E1',
              }}
            >
              <span

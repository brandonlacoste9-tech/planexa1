/** Demo catalog for booking UI and `booking.getCatalog` until DB-backed data exists. */
...
export const DEMO_BOOKING_SLUGS = [businessInfo.slug] as const;

export function getDemoCatalogForSlug(
  slug: string
):
  | { found: false }
  | { found: true; business: BookingBusiness; appointmentTypes: AppointmentType[] } {
  const n = slug.trim().toLowerCase();
  if (n === businessInfo.slug.toLowerCase()) {
    return { found: true, business: businessInfo, appointmentTypes };
  }
  return { found: false };
}
